            ...
            <div class="card-box pd-30 height-100-p">
                @if($ps)
                    ...
                    @elseif($ps->status == 'Expired')
                        <h2 class="mb-30 h4">Tanda Tangan Expired ❌</h2>
                        Tanda tangan ini telah expired pada tanggal : {{\Carbon\Carbon::parse($ps->created_at)->copy()->addMonths(1)->translatedFormat('l, j F Y \p\u\k\u\l H:i')}}
                    @else
                        <h2 class="mb-30 h4">Status Tidak Diketahui ❓</h2>
                        Status tanda tangan tidak dapat diidentifikasi.
                    @endif
                @else
